(function() {
  $(document).ready(function() {
    ...
    console.log("Loading banners coffeescript");
    ...
    if ($("#editor_container").length > 0) {
      ...
      $("form").submit(function() {
        ...
        return $('textarea[name="banner[htmltemplate]"]').val(code);
      });
    ...
    $('select[name="banner[campaign_id]"').on('change', function() {
      var id;
      id = $(this).val();
    ...
    $('input[type=radio][name=deal_type]').on('change', function() {
      ...
      if (val === "none") {
        ...
        if (val === "private_only") {
          return $('input[name="banner[bid_ecpm]"]').val("0");
        }
      }
    });
